...
type ElementPath =
    ...
    | 'background.layer1'
    | 'background.layer2'
    | 'background.blurLayer1'
    | 'background.blurLayer2'
    | 'weather.container'
    | 'weather.leftContainer'
    | 'weather.rightContainer'
    | 'weather.icon'
    | 'weather.temp'
    | 'weather.text'
    | 'weather.feels'
    ...
    | 'weather.airQuality'
    | 'weather.airValue'
    | 'weather.alertContainer'
    | 'weather.precipContainer'
    | 'weather.precipLabel'
    | 'weather.precipTimes'
    | 'weather.precipValues'
    | 'weather.tip'
    | 'player.control'
    | 'player.title'
    | 'player.artist'
    ...
    | 'playerControl.background'
    | 'playerControl.thumbnail'
    | 'playerControl.thumbnailWrap'
    | 'playerControl.info'
    | 'playerControl.title'
    | 'playerControl.artist'
    | 'playerControl.albumTitle'
    | 'playerControl.timeline'
    | 'playerControl.aubar'
    | 'version.modal'
    | 'version.linkNotificationContainer'
    | 'version.closeBtn'
    | 'version.understandBtn'
    | 'version.dontShowBtn'
    | 'debug.modal'
    | 'debug.textarea'
    | 'pageTitle';
...
function resetElements(): void {
    ...
    elements.clock.indicators = document.querySelector(
        '#clock .clock-block .time-indicators'
    ) as HTMLElement;
    elements.clock.sec = document.querySelector(
        '#clock .clock-block .time-indicators .sec'
    ) as HTMLElement;
    elements.clock.st = document.querySelector(
    ...
    elements.slide.description = document.querySelector(
    ...
    elements.background.blurLayer1 = document.querySelector(
        '#background-blur-layer1'
    ) as HTMLElement;
    elements.background.blurLayer2 = document.querySelector(
        '#background-blur-layer2'
    ) as HTMLElement;

    elements.weather.weather = document.querySelector('#weather') as HTMLElement;
    elements.weather.container = document.querySelector(
        '#weather .weather-container'
    ) as HTMLElement;
    elements.weather.leftContainer = document.querySelector('.weather-left') as HTMLElement | null;
    elements.weather.rightContainer = document.querySelector(
        '.weather-right'
    ) as HTMLElement | null;
    ...
    elements.weather.alertContainer = document.getElementById(
        'weatherAlertContainer'
    ) as HTMLElement;
    elements.weather.precipContainer = document.getElementById(
        'weatherPrecipContainer'
    ) as HTMLElement;
    ...
    elements.playerControl.background = document.querySelector(
        '#player_control .background'
    ) as HTMLElement;
    elements.playerControl.thumbnail = document.querySelector(
        '#player_control .thumbnail'
    ) as HTMLImageElement;
    elements.playerControl.thumbnailWrap = document.querySelector(
        '#player_control .thumbnail-wrap'
    ) as HTMLElement;
    ...
    elements.playerControl.artist = document.querySelector(
        '#player_control .artist'
    ) as HTMLElement;
    elements.playerControl.albumTitle = document.querySelector(
        '#player_control .albumTitle'
    ) as HTMLElement;
    elements.playerControl.timeline = document.querySelector(
        '#player_control .progress-bar'
    ) as HTMLElement;
    elements.playerControl.aubar = document.querySelector(
        '#player_control .aubar'
    ) as HTMLCanvasElement;

    elements.version.modal = document.getElementById('version-modal');
    elements.version.linkNotificationContainer = document.getElementById(
 ...
 * @param timeout 超时时间（毫秒），默认 5000
 */
function waitForElementCallback(
    path: ElementPath,

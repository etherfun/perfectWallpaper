...
// 构建日期字符串
function buildDateString(year, month, day, week, sep) {
    var parts = [];
    var partTypes = [];
    
    // 根据顺序添加年份、月份、日期，并记录类型
    if (dateFormat.order === 1) { // 年月日顺序
    ...
    } else if (dateFormat.order === 2) { // 月日年顺序
        if (month) { parts.push(month); partTypes.push('month'); }
        if (day) { parts.push(day); partTypes.push('day'); }
        if (year) { parts.push(year); partTypes.push('year'); }
    } else if (dateFormat.order === 3) { // 日月年顺序
        if (day) { parts.push(day); partTypes.push('day'); }
        if (month) { parts.push(month); partTypes.push('month'); }
        if (year) { parts.push(year); partTypes.push('year'); }
    }

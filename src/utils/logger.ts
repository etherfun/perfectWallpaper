...
export class DebugLogger {
    ...
    log(message: string, level: number | string = 'INFO', extraData: any = null): LogEntry {
        ...
        const levelNum =
            typeof level === 'string'
                ? this.levels[level.toUpperCase() as keyof typeof this.levels] || 1
                : level;
        const levelName =
            typeof level === 'string' ? level.toUpperCase() : this.getLevelName(level);
    ...
    private setupConsoleRedirection(): void {
        ...
        if (
            this.consoleCaptureConfig.captureGroupCollapsed &&
        ...
        if (safeConsoleLog) {
            try {
                ...
                if (
                    ...
                    console.log(
                        `[安全模式] [${logEntry.timeString}] [${logEntry.levelName}] ${logEntry.message}`
                    );
                }
            }
        } else {
            ...
            if (
                ...
                console.log(
                    `[备用模式] [${logEntry.timeString}] [${logEntry.levelName}] ${logEntry.message}`
                );
            }
        }
    }
     ...
     */
    getConsoleStats(): any {
        const consoleLogs = this.logs.filter(log => log.extraData && log.extraData.method);

...
export async function openmeteo(
  ...
  weather_data.temperature_min = res.daily.temperature_2p_min?.[0] || res.daily.temperature_2m_max[0];
  ...
  // 天气状况
  weather_data.weathernow = i18n(`weather_openmeteo_${res.current.weather_code}`) || i18n("weather_no_data");
  ...
  weather_data.rangetemperature = `${res.daily.temperature_2p_min?.[0] || res.daily.temperature_2m_max[0]}~${res.daily.temperature_2m_max[0]}`;
  weather_data.uvindex = res.daily.uv_index_max?.[0] || i18n("weather_no_data");
  weather_data.rangefeelstemperature = `${res.daily.apparent_temperature_min?.[0] || res.daily.temperature_2p_min?.[0] || res.daily.temperature_2m_max[0]}~${res.daily.apparent_temperature_max?.[0] || res.daily.temperature_2m_max[0]}`;
  ...
  if (res.hourly && res.hourly.time && res.hourly.time.length > 0) {
    const now = new Date();
    const currentTime = now.toISOString().split('T')[0] + 'T' + now.getHours().toString().padStart(2, '0') + ':00';
    ...
    for (let i = 0; i < next7Hours; i++) {
      ...
      sevenHourlyData.WindSpeeds.push(res.hourly.wind_speed_10m?.[idx] ?? res.current.wind_speed_10m);
      sevenHourlyData.Humidities.push(res.hourly.relative_humidity_2m?.[idx] ?? res.current.relative_humidity_2m);
      sevenHourlyData.Precips.push(res.hourly.precipitation?.[idx] ?? res.current.precipitation);
      sevenHourlyData.Pressures.push(res.hourly.surface_pressure?.[idx] ?? res.current.pressure_msl);
